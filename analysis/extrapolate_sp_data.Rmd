...
1. For each species id, get a mean and then extrapolate the sd from that. This raises the ?, what to do when some records for a species have an sd and some do not? 
2. Or, for every mean without an sd, extrapolate an sd. Then get average mean and sd for each species.

For now I am going to implement number 2. 

```{r extrapolate sds if missing}
